...
def make_root_json(name, tile_format, min_zoom, max_zoom, bounds_lonlat) -> dict:
    ...
    extent = {"xmin": xmin, "ymin": ymin, "xmax": xmax, "ymax": ymax, "spatialReference": sr}
    return {
        "tileImageInfo": {"format": tile_format, "compressionQuality": 0 if tile_format == "PNG" else 90},
        "name": name,
        "version": 1.0,
        ...
        "resampling": True,
        "tileInfo": {"rows": 256, "cols": 256, "dpi": DPI, "origin": {"x": WEB_MERCATOR_ORIGIN_X, "y": WEB_MERCATOR_ORIGIN_Y}, "spatialReference": sr, "lods": lods},
        "exportTilesAllowed": False,
        "storageInfo": {"storageFormat": "esriMapCacheStorageModeCompactV2", "packetSize": PACKET_SIZE},
        "initialExtent": extent,
        "fullExtent": extent,
    }


def make_iteminfo_json(name, bounds_lonlat, metadata) -> dict:
    west, south, east, north = bounds_lonlat
    ...
    return {
        "creator": "MBTiles_to_TPKX", "name": name, "guid": str(uuid.uuid4()).upper(),
        "version": 1.0, "created": int(time.time() * 1000), "snippet": "Converted from MBTiles",
        "description": description, "summary": "Raster tile package converted from MBTiles", "title": name,
        "tags": ["MBTiles", "TPKX"], "type": "Compact Tile Package",
        "typeKeywords": ["Compact Tile Package", "Tile Package", "tpkx"], "thumbnail": "./thumbnail.png",
        "extent": {"xmin": west, "ymin": south, "xmax": east, "ymax": north, "spatialReference": {"wkid": 4326, "latestWkid": 4326}},
    }
...
class BundleWriter:
    def __init__(self, path: Path):
        ...
        header = struct.pack("<4I3Q4I", 3, TILES_PER_BUNDLE, 0, 0, 0, 0, 0, 0, 0, 40, 1)
        if len(header) != 64:
            raise ConversionError("Internal bundle header size error.")
    ...
    def add_tile(self, row: int, col: int, tile_data: bytes) -> None:
        ...
        if tile_size >= (1 << 24):
            raise ConversionError(f"Tile too large for Compact Cache V2: {tile_size} bytes")
...
def build_bundles(conn, work_root: Path, tile_count: int, progress_every: int = 5000) -> int:
    ...
    query = """
        ...
        ORDER BY zoom_level,
            ...
            (tile_column / 128),
            ((1 << zoom_level) - 1 - tile_row), tile_column
    """
    current_key = None
    current_bundle = None
    processed = 0
    ...
    try:
        for z, x, tms_y, tile_data in conn.execute(query):
            z, x, tms_y = int(z), int(x), int(tms_y)
            y = (1 << z) - 1 - tms_y
            ...
            if processed == 1 or processed % progress_every == 0 or processed == tile_count:
                ...
                print(f"Tiles {fmt_int(processed)} / {fmt_int(tile_count)} ({pct:6.2f}%)  Bundles {fmt_int(bundle_count)}  Elapsed {format_elapsed(time.monotonic() - started)}", flush=True)
    finally:
        if current_bundle is not None:
            current_bundle.close()
    return bundle_count
...
def package_tpkx(work_root: Path, output_path: Path) -> None:
    if output_path.exists():
        output_path.unlink()
    files = sorted([p for p in work_root.rglob("*") if p.is_file()], key=lambda p: str(p.relative_to(work_root)).lower())
...
def convert_mbtiles_to_tpkx(input_path: Path, output_path: Path, keep_work: bool = False) -> None:
    ...
    try:
        ...
        try:
            ...
            print(f"Name  : {name}\nFormat: {tile_format}\nZooms : {min_zoom}-{max_zoom}\nTiles : {fmt_int(tile_count)}")
            print("Bounds: " f"{bounds[0]:.8f}, {bounds[1]:.8f}, {bounds[2]:.8f}, {bounds[3]:.8f}\n")
            (work_root / "root.json").write_text(json.dumps(make_root_json(name, tile_format, min_zoom, max_zoom, bounds), ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
            (work_root / "iteminfo.json").write_text(json.dumps(make_iteminfo_json(name, bounds, metadata), ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
        ...
        print(f"\nCONVERSION COMPLETE\nTiles   : {fmt_int(tile_count)}\nBundles : {fmt_int(bundle_count)}\nOutput  : {output_path}\nBytes   : {fmt_int(output_path.stat().st_size)}\nElapsed : {format_elapsed(time.monotonic() - started)}")
    except Exception:
        print(f"\nWorking files preserved at: {work_root}", file=sys.stderr)
        raise
    else:
        if keep_work:
...
def choose_paths_gui() -> tuple[Path | None, Path | None]:
    ...
    try:
        input_name = filedialog.askopenfilename(title="Select MBTiles file", filetypes=[("MBTiles", "*.mbtiles"), ("All files", "*.*")])
        if not input_name: return None, None
        input_path = Path(input_name)
        output_name = filedialog.asksaveasfilename(title="Save ArcGIS Tile Package", defaultextension=".tpkx", initialfile=input_path.stem + ".tpkx", filetypes=[("ArcGIS Compact Tile Package", "*.tpkx")])
        if not output_name: return None, None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Convert raster Web Mercator MBTiles to ArcGIS TPKX (Compact Cache V2).")
    parser.add_argument("input", nargs="?", help="Input .mbtiles file")
    parser.add_argument("output", nargs="?", help="Output .tpkx file")
    parser.add_argument("--keep-work", action="store_true", help="Keep the intermediate unpacked Compact Cache V2 folder.")
...
def main() -> int:
    ...
    try:
        ...
        print("Cancelled by user.", file=sys.stderr); return 130
    except Exception as exc:
        print(f"UNEXPECTED ERROR: {type(exc).__name__}: {exc}", file=sys.stderr); return 3
    return 0
